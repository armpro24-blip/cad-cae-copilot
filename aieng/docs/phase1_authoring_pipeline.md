...
This document describes the Phase 1 `.aieng` **agent-selectable authoring workflow**: the end-to-end flow an agent or user can invoke to turn natural-language intent into a versioned, auditable engineering data package.

> **Important:** The authoring pipeline is **not** an unconditionally auto-triggered system behaviour. It is an **opt-in workflow** that an agent or CLI user explicitly chooses when the goal is to create a new CAD model from intent. For existing CAD/CAE files, use import, validate, topology extraction, summarise, or analysis flows instead.
...
Phase 1 provides an **explicit create-new-CAD-model workflow**. An agent or user runs the following end-to-end flow only when the intent is to generate new geometry from scratch:
...
- The user wants to **create a new CAD model** from natural-language intent.
- The agent needs a **reproducible, auditable package** with frozen intent, construction history, and evidence.
- The environment has a compliant backend (FakeBackend for CI/tests, FreeCAD for real geometry).

### When NOT to use this workflow

- The input is an **existing CAD file** (STEP, IGES, etc.). Use import → validate → extract topology → summarise instead.
- The input is an **existing CAE deck** (Abaqus `.inp`, etc.). Use CAE import → parse → map → analysis flows instead.
- The agent only needs to **inspect or annotate** an existing `.aieng` package. Use package validation, context application, or visual annotation tools instead.
- The user wants a **quick geometric query** without provenance. The authoring pipeline intentionally carries overhead (plan validation, evidence, trace) that may be unnecessary for one-off queries.
...
- **Topology / feature graph post-processing** — Implemented in PR 5. Run by default when `init_from_plan()` is explicitly invoked and the backend successfully exports geometry. Can be disabled with `run_postprocess=False` or `--no-postprocess`.
...
### PR 5: Semantic post-processing (Completed)

When `init_from_plan()` is explicitly called and the backend successfully exports geometry, the orchestrator optionally runs a semantic post-processing chain before returning:
...
Gated by:
- `run_postprocess: bool = True` (default) — disable with `--no-postprocess` for fast generation, CI, environments without OCP, diagnostic-only packages, or when the agent only needs authoring traces.
- `postprocess_strict: bool = False` (default) — if `True`, any post-processing failure raises an exception.

Diagnostic packages (`partial` / `failed`) always skip semantic post-processing.
