    ...
    "opt.run_topology_optimization": {
        ...
        "properties": {
            ...
            "design_space_node": {
                "type": "string",
                "description": (
                    "Body to optimize when deriving (default: the largest single solid). Pass "
                    "'whole_model' for the envelope spanning every body — needed when the load "
                    "enters through a different part than the one being optimized. A refusal "
                    "lists the valid names in design_space_candidates."
                ),
            },
            "dimension": {
                "type": "string",
    ...
    "generate_bom": {
        ...
        "properties": {
            ...
            "format": {
                ...
                "description": "Output format. Structured `items` are always returned; markdown adds a table string, csv adds an ERP-style line-item CSV (result.csv), json adds a serialized ERP line-item document (result.json), and xlsx adds a base64-encoded minimal workbook (result.xlsx_base64).",
            },
        },
        "additionalProperties": False,
